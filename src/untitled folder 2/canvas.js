...
[
  ...
  'clusters','clusterTiles','clusterCount','clusterCountVal','clusterRadius','clusterRadiusVal','spatialGap','spatialGapVal',
...
function updateLabels() {
  ...
  if (els.glitchAlpha)      els.glitchAlphaVal.textContent      = (+els.glitchAlpha.value).toFixed(2);
  if (els.glitchJitter)     els.glitchJitterVal.textContent     = (+els.glitchJitter.value).toFixed(2);
  if (els.glitchSmearAngle) els.glitchSmearAngleVal.textContent = (els.glitchSmearAngle.value|0) + '°';
  
  // inside updateLabels()
  if (els.scanAlpha)  els.scanAlphaVal.textContent  = (+els.scanAlpha.value).toFixed(2);
  if (els.scanShift)  els.scanShiftVal.textContent  = (+els.scanShift.value).toFixed(2);
  ...
  if (els.solarizeR)      els.solarizeRVal.textContent      = (+els.solarizeR.value).toFixed(2);
  if (els.solarizeG)      els.solarizeGVal.textContent      = (+els.solarizeG.value).toFixed(2);
  if (els.solarizeB)      els.solarizeBVal.textContent      = (+els.solarizeB.value).toFixed(2);
}
