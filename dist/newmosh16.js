/* p5 Datamosh — Cycle Toggle + Bloom/Flow/Corrupt toggles + Speed fine
   Based on your uploaded version.
*/
let videoEl;
let gCur, gBuf;
let gWarp, gBloomWork;
let gTemp;
...
function hookUI() {
    ...
    els.corruptOn = $('corruptOn'); // NEW
    els.corrupt = $('corrupt');
    els.corruptVal = $('corruptVal');
    els.block = $('block');
    els.blockVal = $('blockVal');
    els.glitchSpeed = $('glitchSpeed');
    els.glitchSpeedVal = $('glitchSpeedVal');
    els.glitchSpeedFine = $('glitchSpeedFine');
    els.glitchSpeedFineVal = $('glitchSpeedFineVal'); // NEW
    els.glitchSize = $('glitchSize');
    els.glitchSizeVal = $('glitchSizeVal');
    els.glitchSmear = $('glitchSmear');
    els.glitchSmearVal = $('glitchSmearVal');
    // determinism & evolution
    els.seed = $('seed');
    els.feedback = $('feedback');
    els.feedbackVal = $('feedbackVal');
    els.persistence = $('persistence');
    els.persistenceVal = $('persistenceVal');
    els.fbX = $('fbX');
    els.fbXVal = $('fbXVal');
    els.fbY = $('fbY');
    els.fbYVal = $('fbYVal');
    ...
    els.fbZVal = $('fbZVal');
    els.fbTheta = $('fbTheta');
    els.fbThetaVal = $('fbThetaVal');
    // spatial/clusters
    els.spatialGap = $('spatialGap');
    els.spatialGapVal = $('spatialGapVal');
    els.clusters = $('clusters');
    els.clusterCount = $('clusterCount');
    els.clusterCountVal = $('clusterCountVal');
    els.clusterRadius = $('clusterRadius');
    els.clusterRadiusVal = $('clusterRadiusVal');
    // cycle + bursts
    els.cycleOn = $('cycleOn');
    els.cycleShape = $('cycleShape');
    els.burstOn = $('burstOn');
    els.burstLen = $('burstLen');
    els.burstLenVal = $('burstLenVal');
    els.burstGap = $('burstGap');
    els.burstGapVal = $('burstGapVal');
    els.burstBoost = $('burstBoost');
    els.burstBoostVal = $('burstBoostVal');
    // Bloom + Flow
    els.bloomOn = $('bloomOn'); // NEW
    els.bloomStrength = $('bloomStrength');
    els.bloomStrengthVal = $('bloomStrengthVal');
    els.bloomRadius = $('bloomRadius');
    els.bloomRadiusVal = $('bloomRadiusVal');
    els.flowOn = $('flowOn'); // NEW
    els.flowStrength = $('flowStrength');
    els.flowStrengthVal = $('flowStrengthVal');
    els.flowScale = $('flowScale');
    els.flowScaleVal = $('flowScaleVal');
    // Base
    els.baseOn = $('baseOn');
    els.baseMix = $('baseMix');
    els.baseMixVal = $('baseMixVal');
    els.seedOnLoad = $('seedOnLoad');
    // Pixel Sort
    els.psOn = $('psOn');
    els.psDir = $('psDir');
    els.psWin = $('psWin');
    els.psWinVal = $('psWinVal');
    els.psThresh = $('psThresh');
    els.psThreshVal = $('psThreshVal');
    els.psChan = $('psChan');
    // Colorizer
    els.colOn = $('colOn');
    els.colHue = $('colHue');
    els.colHueVal = $('colHueVal');
    els.colSat = $('colSat');
    els.colSatVal = $('colSatVal');
    ...
    [
        'depth', 'corrupt', 'block',
        'glitchSpeed', 'glitchSpeedFine', // NEW
        'glitchSize', 'glitchSmear',
    ...
    els.cycleShape.addEventListener('change', updateLabels);
    els.psDir.addEventListener('change', updateLabels);
    els.psChan.addEventListener('change', updateLabels);
}
...
function updateLabels() {
    ...
    els.glitchSpeedFineVal.textContent = f2(els.glitchSpeedFine.value); // NEW
...
function draw() {
    ...
    const fine = parseFloat(els.glitchSpeedFine.value || 1); // NEW multiplier
    ...
    // Pixel Sort (optional)
    if (els.psOn.checked) {
        applyPixelSort(gBuf, gTemp,
            els.psDir.value, parseInt(els.psWin.value, 10),
...
function drawWaiting() {
    ...
    textSize(14);
    text('Load a video (no audio). Bloom/Flow/Corrupt toggles + speed fine added.', width / 2, height / 2);
}
...
function toggleUI() {
    ...
    if (header.style.display === 'none') {
        header.style.display = ''; // restore default
    } else {
...
function keyPressed() {
    if (key === 'f' || key === 'F') {
        if (!document.fullscreenElement) {
            // request fullscreen on the whole page (canvas will resize via windowResized)
